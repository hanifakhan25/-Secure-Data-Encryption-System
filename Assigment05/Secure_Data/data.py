...
import json
import time
from encryption_utils import create_fernet, encrypt_data, decrypt_data, hash_passkey
...
def init_session():
    defaults = {
        'stored_data': {},
        'failed_attempts': 0,
        'authenticated': False,  # Changed from 'logged_in' to more professional term
...
if choice == "Home":
    ...
    st.markdown("""
    - 🔒 **Secure Storage**: Encrypt sensitive data with military-grade encryption
        ...
        if st.button("Secure Data", key="encrypt_btn"):
            if all([user_text, user_passkey, entry_name]):
                with st.spinner("🔐 Encrypting data..."):
                    ...
                    encrypted = encrypt_data(st.session_state.fernet, user_text)
    ...
    if st.button("Decrypt Data"):
        ...
        with st.spinner("🔓 Decrypting..."):
            ...
            result = decrypt_data(st.session_state.fernet, selected_label, key_input, st.session_state.stored_data)
